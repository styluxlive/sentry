...
def build_sdk_crash_detection_configs() -> Sequence[SDKCrashDetectionConfig]:
    configs: Sequence[SDKCrashDetectionConfig] = []

    cocoa_options = _get_options(sdk_name=SdkName.Cocoa, has_allowlist=False)
    ...
    react_native_options = _get_options(sdk_name=SdkName.ReactNative, has_allowlist=True)
...
def _get_options(sdk_name: SdkName, has_allowlist: bool) -> Optional[SDKCrashDetectionOptions]:
