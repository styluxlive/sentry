...
@django_db_all
@pytest.mark.snuba
def test_strip_event_data_strips_value_if_not_simple_type(store_event, sdk_crash_detection_configs):
    event = store_event(data=get_crash_event())
    event.data["type"] = {"foo": "bar"}

    stripped_event_data = strip_event_data(
        event.data, SDKCrashDetector(config=sdk_crash_detection_configs[0])
    )
...
@django_db_all
@pytest.mark.snuba
def test_strip_event_data_keeps_simple_types(store_event, sdk_crash_detection_configs):
    event = store_event(data=get_crash_event())
...
@django_db_all
@pytest.mark.snuba
def test_strip_event_data_keeps_exception_mechanism(store_event, sdk_crash_detection_configs):
    event = store_event(data=get_crash_event())
...
def test_strip_frames_sdk_frames_keep_after_matcher(
    store_and_strip_event, sdk_crash_detection_configs
):
    frames = get_frames("SentryCrashMonitor_CPPException.cpp", sentry_frame_in_app=False)
    ...
    config = sdk_crash_detection_configs[1]
