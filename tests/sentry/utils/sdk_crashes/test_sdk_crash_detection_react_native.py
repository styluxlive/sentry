...
@decorators
def test_sdk_crash_is_reported(
    mock_sdk_crash_reporter, mock_random, store_event, sdk_crash_detection_configs
):
    event = store_event(data=get_crash_event())

    sdk_crash_detection_configs[1].organization_allowlist = [event.project.organization_id]

    sdk_crash_detection.detect_sdk_crash(event=event, configs=sdk_crash_detection_configs)
...
def test_sdk_crash_sample_app_not_reported(
    ...
    sdk_crash_detection.detect_sdk_crash(
        event=event,
        configs=sdk_crash_detection_configs,
    )
...
def test_organization_not_in_allowlist_not_detected(
    ...
    sdk_crash_detection_configs[1].organization_allowlist = [event.project.organization_id + 1]
...
def test_organization_empty_allowlist_not_detected(
    ...
    # sdk_crash_detection_configs[1].organization_allowlist = []
